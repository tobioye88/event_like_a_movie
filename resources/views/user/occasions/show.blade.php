<x-user.layout :title="$occasion->title">
  <x-slot:actions>
    <div class="flex flex-wrap gap-2">
      <a href="{{ route('invites.show', $occasion) }}" target="_blank"
        class="rounded border border-slate-300 px-4 py-2 hover:bg-slate-100">Open Invite</a>
      @if($occasion->status !== 'active')
      <form method="POST" action="{{ route('occasions.publish', $occasion) }}">
        @csrf
        @method('PATCH')
        <button type="submit" class="rounded bg-primary px-4 py-2 text-white hover:bg-primary-dark">Publish
          Occasion</button>
      </form>
      @endif
      <a href="{{ route('occasions.edit', $occasion) }}"
        class="rounded bg-slate-900 px-4 py-2 text-white hover:bg-slate-800">Edit</a>
    </div>
  </x-slot:actions>
  ...
  <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    <section class="rounded-lg border border-slate-200 bg-white p-6 lg:col-span-1">
      ...
      <input readonly value="{{ route('invites.show', $occasion) }}"
      ...
      <form method="POST" action="{{ route('occasions.destroy', $occasion) }}" class="mt-6"
        onsubmit="return confirm('Delete this occasion?')">
        @csrf
        @method('DELETE')
        <button type="submit" class="rounded border border-red-300 px-4 py-2 text-red-700 hover:bg-red-50">Delete
          Occasion</button>
      </form>
    </section>
          ...
          <tbody class="divide-y divide-slate-200">
            ...
            <tr>
              ...
              <td class="px-4 py-3">
                ...
                @if($rsvp->guest_count && $rsvp->guest_count > 0)
                <div>{{ $rsvp->guest_count }} {{ Str::plural('guest', $rsvp->guest_count) }}</div>
                @endif
              </td>
